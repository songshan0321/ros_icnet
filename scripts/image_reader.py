...
def _brightness_shift(img, label):
    img = tf.image.random_brightness(img, max_delta=50, seed=None)

    return img, label

def _image_cropping(img, label):
    image_shape = tf.shape(img)
    img = tf.image.crop_to_bounding_box(img, 288, 0, 432, image_shape[1])
    label = tf.image.crop_to_bounding_box(label, 288, 0, 432, image_shape[1])
...
class ImageReader(object):
    ...
    def create_tf_dataset(self, cfg):
        ...
        if cfg.upper_crop:
            dataset = dataset.map(_image_cropping, num_parallel_calls=cfg.N_WORKERS)
        ...
        if cfg.is_training: # Training phase
            ...
            if cfg.random_brightness:
                dataset = dataset.map(_brightness_shift, num_parallel_calls=cfg.N_WORKERS)
